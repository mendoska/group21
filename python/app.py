    ...
    for targetID in targetList:
        ...
        if targetThreat.currentStatus == "Alive":
            if weaponModel.ammunitionQuantity <= 0:
                ic(f"{weaponModel.weaponName} has run out of ammunition, Cannot fire at Drone {targetID}")
                continue
...
def droneController(droneID:int, droneDirectory:dict, subprocesses:dict, spawnRange:set, locationDict:dict, spawnParams:set) -> None:
    ...
    """ If threat passes both checks (next 2 lines), the threat will be removed from the simulation and treated as a leaker """
    ...
    ic(f"Drone {droneID} Process Leaked and Killed")
    return 
...
def run_BOWSER_simulation(spawnRange:set, algorithmChoice:str, numberOfDrones:int) -> tuple:
    ...
    for weaponDelegationIndex in range(len(response[0][1])):
        ...
        for thread in weaponThreads: # Suspend the loop until all weapons systems completed
            thread.join()
    ...
    try: # Try to kill remaining drone subprocessess
