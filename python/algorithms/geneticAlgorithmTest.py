    ...
    if ga_instance.generations_completed == ga_instance.num_generations :
        for solution in range(len(ga_instance.best_solution()[0])):
            ...
            if ga_instance.best_solution()[0][solution] == 1:
                res.append(f"[THREAT: {function_inputs[solution]}, Weapon Chosen: 'Long Range Missile']")
                continue
            if ga_instance.best_solution()[0][solution] == 2:
                res.append(f"[THREAT: {function_inputs[solution]}, Weapon Chosen: 'Medium Range Missile']")
                continue
            if ga_instance.best_solution()[0][solution] == 3:
                res.append(f"[THREAT: {function_inputs[solution]}, Weapon Chosen: 'Short Range Missile']")
                continue
            if ga_instance.best_solution()[0][solution] == 4:
                res.append(f"[THREAT: {function_inputs[solution]}, Weapon Chosen: 'Directed Energy']")
                continue
